...
import { DEFAULT_SITE_ORIGIN } from '../src/seo.js'
...
expectIncludes(about, `<link rel="canonical" href="${siteOrigin}/about" />`, 'about canonical')
...
expectIncludes(product, `${siteOrigin}/export-product/s-s-shea-butter`, 'product canonical')
expectIncludes(product, '"@type":"ItemPage"', 'product page JSON-LD')
...
expectIncludes(robots, `Sitemap: ${siteOrigin}/sitemap.xml`, 'robots sitemap location')

const sitemap = await read('dist/sitemap.xml')
expectIncludes(sitemap, `${siteOrigin}/export-product/s-s-shea-butter`, 'sitemap product URL')
...
console.log(`PASS SEO prerender, metadata, sitemap, robots and redirects for ${siteOrigin}`)
